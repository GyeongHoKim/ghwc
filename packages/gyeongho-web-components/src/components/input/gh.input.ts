...
import { GHMenuItem } from "../menu-item/gh.menu.item.ts";
...
export class GHInput extends LitElement {
    ...
    return [
      ...
      css`
        ...
        .clear-icon {
          transition:
            var(--transition-1),
            0.1s opacity ease-out 0.1s;
        }

        :host(:not(:hover):not([active])) .clear-icon {
          transition:
            var(--transition-1),
            0.1s width ease-out 0.1s,
  ...
  render() {
    return html`
      ${this.icon
        ? html` <gh-icon class="icon" icon="${this.icon}"></gh-icon> `
        : ""}
      <div class="center">
      ...
      <!-- date -->
      ${this.type === "date"
        ? html` <gh-icon button class="date-icon" icon="event"></gh-icon> `
        : ""}
      <!-- clear -->
      ${!this.disabled &&
      !this.readonly &&
  ...
  handleBlur(e: FocusEvent) {
    if (this.type === "number") {
      this.validateMinMax(Number((<HTMLInputElement>e.target)?.value));
    }
    if (this.type !== "select") {
      this.active = false;
    }
  ...
  handleItems(e: Event) {
    const items: Node[] = (<HTMLSlotElement>e.target)?.assignedNodes();
    items.forEach((el: Node) => {
      if (el instanceof HTMLElement && el.tagName === "GH-MENU-ITEM") {
        // handle click on menu item
        (<GHMenuItem>el).addEventListener("active-changed", (e: Event) => {
          if ((<GHMenuItem>e.target)?.active) {
            // unselect siblings
            items.forEach((item) => {
              if (!(item instanceof GHMenuItem)) {
                return;
              }
              if (item !== e.target) {
            ...
            this.value = (<GHMenuItem>el).label;
            this.active = false;
          }
        });
      }
